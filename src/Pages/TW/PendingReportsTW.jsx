...
function PendingReportsTW() {
     ...
     useEffect(() => {
          setMyProject(dashboardProjects?.filter((el) => el[`Project_Assigned_Form.TW`] === user?.name && el['PS_2.IDP_Approved_by_EM'].length > 5 && el[`PS_3.Draft_Report_Start_Date`].length >= 2));
     }, [dashboardProjects]);
          ...
          <Box bg={'white'} mt={10} width='95%' m={'auto'} p={5} borderRadius={'10px'}>
               {
                    dashboardProjectsLoading ? (
                         ...
                         fePendingReportsTWProjects && fePendingReportsTWProjects?.length > 0 ? (
                              <CustomTable columns={allProjectsTableColumns} data={fePendingReportsTWProjects ? fePendingReportsTWProjects : []} searchEnabled={true} />
                         ) : (
...
export default PendingReportsTW;
