...
function ApprovedFbTW() {
     ...
     console.log({ feApprovedFbTWProjects });
     ...
     useEffect(() => {
          setMyProject(dashboardProjects?.filter((el) => el[`Project_Assigned_Form.TW`] === user?.name && el['PS_2.IDP_Approved_by_EM'].length > 5));
     }, [dashboardProjects]);
          ...
          <Box bg={'white'} mt={10} width='95%' m={'auto'} p={5} borderRadius={'10px'}>
               {
                    dashboardProjectsLoading ? (
                         ...
                         feApprovedFbTWProjects && feApprovedFbTWProjects?.length > 0 ? (
                              <CustomTable columns={allProjectsTableColumns} data={feApprovedFbTWProjects ? feApprovedFbTWProjects : []} searchEnabled={true} />
                         ) : (
