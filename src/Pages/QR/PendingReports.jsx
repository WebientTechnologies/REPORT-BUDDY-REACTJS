...
function PendingReports() {
     ...
     console.log({ fePendingReportsProjects });
     ...
     useEffect(() => {
          setMyProject(dashboardProjects?.filter((el) => el[`Project_Assigned_Form.QR`] === user?.name && el['PS_3.Draft_Report_Completed'].length > 5));
     }, [dashboardProjects]);
          ...
          <Box bg={'white'} mt={10} width='95%' m={'auto'} p={5} borderRadius={'10px'}>
               {
                    dashboardProjectsLoading ? (
                         ...
                         fePendingReportsProjects && fePendingReportsProjects?.length > 0 ? (
                              <CustomTable columns={allProjectsTableColumns} data={fePendingReportsProjects ? fePendingReportsProjects : []} searchEnabled={true} />
                         ) : (
