...
function ApprovedProjectsOM() {
     ...
     useEffect(() => {
          setMyProject(dashboardProjects?.filter((el) => el[`Project_Assigned_Form.Operations_Manager_Email`] === user?.email && el['PS_2.IDP_Approved_by_EM'].length > 5));
     }, [dashboardProjects]);
          ...
          <Box bg={'white'} mt={10} width='95%' m={'auto'} p={5} borderRadius={'10px'}>
               {
                    dashboardProjectsLoading ? (
                         ...
                         myApprovedProjects && myApprovedProjects?.length > 0 ? (
                              <CustomTable columns={allProjectsTableColumns} data={myApprovedProjects ? myApprovedProjects : []} searchEnabled={true} />
                         ) : (
...
export default ApprovedProjectsOM;
